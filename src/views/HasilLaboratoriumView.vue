<template>
  <div class="max-w-4xl mx-auto space-y-5 sm:space-y-6" style="animation: fade-in 0.3s ease-out;">
    <!-- Banner Header -->
    <div class="bg-gradient-to-r from-teal-700 via-emerald-700 to-cyan-800 rounded-3xl p-4 sm:p-6 text-white shadow-lg shadow-teal-950/10 relative overflow-hidden">
      ...
      <div class="relative z-10 flex items-center justify-between gap-3 sm:gap-4">
        ...
        <div class="flex items-center gap-3 sm:gap-3.5 min-w-0">
          ...
          <div class="min-w-0 space-y-0.5">
            ...
            <p class="text-[11px] sm:text-xs text-teal-200/90 font-medium truncate">
              Arsip & berkas hasil laboratorium pasien
            </p>
          </div>
        </div>
    ...
    <div v-if="loading" class="py-16 text-center flex flex-col items-center justify-center gap-3 text-gray-400">
      <svg class="w-9 h-9 animate-spin text-teal-600" viewBox="0 0 24 24" fill="none">
        <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3" class="opacity-25"/>
        <path d="M4 12a8 8 0 018-8" stroke="currentColor" stroke-width="3" stroke-linecap="round" class="opacity-75"/>
      </svg>
      <span class="text-xs font-semibold">Memuat riwayat pemeriksaan laboratorium...</span>
    </div>
    ...
    <!-- List of Laboratorium Examinations (Infinite Scroll) -->
    <div v-else-if="listPemeriksaan.length > 0" class="space-y-3.5">
      <div
        v-for="item in listPemeriksaan"
        :key="item.noKunjungan || item.orderNumber || item.id"
        class="bg-white rounded-2xl p-4 sm:p-5 border border-gray-100/90 shadow-sm hover:shadow-md hover:border-teal-200 transition-all duration-200 group"
      >
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <!-- Left: Icon & Detail Pemeriksaan -->
          <div class="flex items-start gap-3.5 flex-1 min-w-0">
            <div class="w-13 h-13 sm:w-14 sm:h-14 rounded-2xl bg-teal-50 border border-teal-100 flex items-center justify-center shrink-0 text-teal-600 group-hover:bg-teal-100/80 transition-colors">
            ...
            <div class="space-y-1.5 flex-1 min-w-0">
              <div class="flex flex-wrap items-center gap-2">
                <h4 class="text-sm sm:text-base font-bold text-gray-900 group-hover:text-teal-700 transition-colors leading-tight break-words">
                  {{ item.pemeriksaanOrder || item.pemeriksaan || 'Pemeriksaan Laboratorium' }}
                </h4>
                <span
                  class="px-2 py-0.5 rounded-md text-[10px] font-bold border inline-flex items-center gap-1 shrink-0"
                  :class="hasValidResult(item) ? 'bg-emerald-50 text-emerald-700 border-emerald-200/60' : 'bg-amber-50 text-amber-700 border-amber-200/60'"
                >
                  <span class="w-1.5 h-1.5 rounded-full" :class="hasValidResult(item) ? 'bg-emerald-500' : 'bg-amber-500'"></span>
                  <span>{{ hasValidResult(item) ? 'Hasil Selesai' : 'Menunggu Validasi' }}</span>
                </span>
              </div>
              ...
              <div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-gray-500">
                <span class="flex items-center gap-1 font-medium">
                  ...
                  <span>{{ formatDateTimeIndo(item.tglOrder || item.tglPemeriksaan) }}</span>
                </span>
                <span class="font-mono text-[11px] text-gray-400">
                  No. Kunjungan: <strong class="text-gray-700 font-semibold">#{{ item.noKunjungan || item.orderNumber || item.id }}</strong>
                </span>
                <span v-if="item.ruanganOrder" class="text-gray-400 hidden sm:inline">
                  • {{ item.ruanganOrder }}
                </span>
              </div>
            </div>
          ...
          <div class="flex items-center gap-2 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-gray-100">
            <button
              ...
              :disabled="downloadingDocId === (item.noKunjungan || item.orderNumber || item.id)"
              class="px-4 py-2.5 rounded-xl bg-teal-600 hover:bg-teal-700 disabled:opacity-50 text-white font-bold text-xs shadow-md shadow-teal-600/20 transition-all flex items-center gap-1.5 active:scale-95 cursor-pointer"
            >
              <svg v-if="downloadingDocId === (item.noKunjungan || item.orderNumber || item.id)" class="w-4 h-4 animate-spin text-white" viewBox="0 0 24 24" fill="none">
                <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3" class="opacity-25"/>
                <path d="M4 12a8 8 0 018-8" stroke="currentColor" stroke-width="3" stroke-linecap="round" class="opacity-75"/>
              </svg>
              ...
              <span>{{ downloadingDocId === (item.noKunjungan || item.orderNumber || item.id) ? 'Mengunduh...' : (isIOS ? 'Unduh Hasil' : 'Hasil') }}</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div ref="loadMoreTrigger" class="py-3 flex flex-col items-center justify-center text-center">
        ...
        <div v-if="loadingMore" class="flex items-center gap-2 py-2 text-xs font-semibold text-teal-600">
          <svg class="w-4 h-4 animate-spin text-teal-600" viewBox="0 0 24 24" fill="none">
            <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3" class="opacity-25"/>
            <path d="M4 12a8 8 0 018-8" stroke="currentColor" stroke-width="3" stroke-linecap="round" class="opacity-75"/>
          </svg>
        ...
        <div v-else-if="!hasMore && listPemeriksaan.length > 0" class="text-[11px] text-gray-400 py-1.5 font-medium flex items-center gap-1.5">
          ...
          <span>Semua riwayat pemeriksaan laboratorium telah ditampilkan</span>
        </div>
      </div>

      <!-- Info Konfirmasi Laboratorium -->
      <div class="p-4 sm:p-5 bg-teal-50/70 border border-teal-200/80 rounded-2xl flex items-start gap-3.5 text-xs text-teal-900 shadow-sm">
        <div class="w-8 h-8 rounded-xl bg-teal-100 text-teal-700 flex items-center justify-center shrink-0 mt-0.5">
          <svg class="w-4.5 h-4.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            <circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/>
          </svg>
        </div>
        <div class="space-y-0.5">
          <h4 class="font-bold text-teal-950">Informasi Hasil Pemeriksaan:</h4>
          <p class="text-teal-800 leading-relaxed">
            Jika dokumen hasil laboratorium belum tersedia atau belum selesai divalidasi oleh analis, silakan <strong>lakukan konfirmasi ke bagian Instalasi Laboratorium RSUD R.A.A. Tjokronegoro</strong>.
          </p>
        </div>
      </div>
    </div>
    ...
    >
      ...
      <div class="space-y-1.5 max-w-md">
        <h3 class="text-base font-bold text-gray-800">Belum Ada Riwayat / Dokumen Laboratorium</h3>
        <p class="text-xs text-gray-400 leading-relaxed">
          Belum ditemukan dokumen hasil laboratorium yang terverifikasi untuk No. Rekam Medis Anda. Jika Anda telah melakukan pemeriksaan atau pengambilan sampel namun dokumen belum terbit, silakan lakukan konfirmasi ke bagian Laboratorium.
        </p>
      </div>
    </div>
        ...
        >
          <div
            ...
            :class="{
              ...
              'bg-teal-600 text-white border-teal-500 shadow-teal-900/20': flashMessage.type === 'info'
            }"
          >
            <!-- Icon -->
            <div class="w-8 h-8 rounded-xl bg-white/20 flex items-center justify-center shrink-0 mt-0.5">
              ...
              <svg v-else-if="flashMessage.type === 'error'" class="w-5 h-5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
...
function hasValidResult(item) {
  if (item.hasHasil === 0 || item.hasHasil === '0' || item.hasHasil === false) {
    return false
  }
  return true
}
...
function getItemTimestamp(item) {
  const dStr = item.tglOrder || item.order_datetime || item.tglPemeriksaan || item.tanggal || item.TANGGAL || item.MASUK || item.masuk || ''
  if (!dStr) return 0
...
async function fetchListPemeriksaan(isInitial = false) {
  ...
  try {
    const response = await api.get('/hasil-laboratorium/list-pemeriksaan', {
      params: {
        page: page.value,
        pageSize: pageSize
      }
    ...
    if (res && (res.code === '200' || res.code === 200)) {
      ...
      if (isInitial) {
        ...
        const existingKeys = new Set(listPemeriksaan.value.map(i => i.noKunjungan || i.orderNumber || i.id))
        for (const item of items) {
          const key = item.noKunjungan || item.orderNumber || item.id
      ...
      if (isInitial) {
        errorMessage.value = res?.message || 'Gagal memuat riwayat laboratorium.'
      }
      hasMore.value = false
    }
  } catch (err) {
    if (err.response?.status === 404) {
      ...
      if (isInitial) {
        errorMessage.value = err.response?.data?.message || 'Terjadi gangguan saat mengambil data laboratorium.'
      }
    }
  } finally {
    loading.value = false
...
/**
 * Membuka Halaman Preview PDF Fullscreen Tersendiri (Dekripsi otomatis)
 */
function openBacaanFullscreen(item) {
  if (item.hasHasil === 0 || item.hasHasil === '0' || item.hasHasil === false) {
    showFlash(
      'Dokumen Belum Tersedia',
      'Hasil dokumen laboratorium belum selesai divalidasi atau belum diarsipkan. Silakan lakukan konfirmasi ke bagian Instalasi Laboratorium.',
      'warning'
    )
    return
  }

  const docId = item.noKunjungan || item.orderNumber || item.id
  if (!docId) return
  ...
  router.push({
    ...
    query: {
      title: item.pemeriksaanOrder || item.pemeriksaan || 'Hasil Pemeriksaan Laboratorium',
      tgl: formatDateTimeIndo(item.tglOrder || item.tglPemeriksaan)
    }
  })
}
...
async function handleDocAction(item) {
  ...
  if (isIOS.value) {
    ...
    try {
      await directDownloadDocument('laboratorium', docId, item.pemeriksaanOrder || item.pemeriksaan || 'Hasil_Laborat')
    } catch (err) {
      showFlash('Gagal Mengunduh Dokumen', err.message || 'Dokumen hasil laboratorium belum tersedia.', 'error')
    } finally {
